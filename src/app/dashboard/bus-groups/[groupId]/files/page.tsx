...
export default function FilesPage({ params }: { params: { groupId: string } }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div>
          ...
          <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
            className="flex items-center gap-2 bg-green-700 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-green-800 disabled:opacity-50">
            {uploading
      ...
      {files.length === 0 ? (
        <div className="bg-white border border-green-100 rounded-2xl p-16 text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {files.map(f => (
            <div key={f.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden hover:border-green-200 transition-colors shadow-sm">
              {f.fileType === "image" ? (
              ...
              <div className="p-3">
                ...
                <div className="flex gap-1 mt-2">
                  <a href={f.fileUrl} download={f.fileName ?? "file"} target="_blank" rel="noreferrer"
                    className="flex-1 flex items-center justify-center gap-1 text-xs bg-green-50 text-green-700 hover:bg-green-100 rounded-lg py-1.5 transition-colors font-medium">
                  ...
                  <button onClick={() => copyUrl(f.id, f.fileUrl)}
                    ...
                    {copiedId === f.id ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3" />}
                  </button>
                </div>
              </div>
            </div>
